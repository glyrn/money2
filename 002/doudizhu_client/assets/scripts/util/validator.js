...
const validateMgr = function(){

    var that = {};

    that.validate_laizi = function(cards,laizis){
        ...
        if (hasLaizi && !is_all_laizi) {
            ...
            check_list.forEach(val => {
                var ret = that.validate_base(val);
                if (ret.status) {
                    if (result) {
            ...
            result = that.validate_base(cards);
        }
        return result;
    }

    that.validate_base = function (cards) {
