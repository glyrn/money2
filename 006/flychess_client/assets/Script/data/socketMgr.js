const socketMgr = function(){
    ...
    that.initSocket = function() {
        ...
        _socket.on('GAME_START',function(data){
            ...
            //debug
            // that.finish_chess(3,0);
            // that.finish_chess(3,1);
            // that.finish_chess(3,2);
            // that.finish_chess(3,3);
        })
    ...
    that.login = function(uid,name,avatorUrl,score,room,play_mode,ready_count,play_count,ob_uid,cbFunc){
        _socket.emit('LOGIN', {uid:uid,room:room,name:name,avatorUrl:avatorUrl,score:score,play_mode:play_mode,ready_count:ready_count,play_count:play_count,ob_uid:ob_uid,lanuch_url:cc.args['lanuch_url'],isRobot:cc.args['isRobot'],robot:cc.args['robot'],robots:cc.args['robots'],auto_ready:cc.args['auto_ready']});
        _cbLogin = cbFunc;
        //是否旁观
