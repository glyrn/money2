...
playLogic.init = function (depth, map){
    var map = map || chessLogic.currMap;
    var depth = depth || 3
    playLogic.my				=	1;				//玩家方
    ...
    chessLogic.createMans( map )		//生成棋子
...
playLogic.regret = function (){
    var map  = chessLogic.arr2Clone(chessLogic.currMap);
    //初始化所有棋子
...
playLogic.clickCanvas = function (x,y){
    console.log("playLogic.isPlay :",playLogic.isPlay)
...
playLogic.reset = function(){
    playLogic.map =	chessLogic.arr2Clone ( chessLogic.currMap );
    playLogic.pace = [];
}
