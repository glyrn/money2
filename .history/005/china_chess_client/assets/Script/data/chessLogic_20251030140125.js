...
chessLogic.init = function (){
    ...
    var style = {
        ...
        spaceX:60,		//着点X跨度
        spaceY:58,		//着点Y跨度
        pointStartX:30,		//第一个着点X坐标;
        pointStartY:29,		//第一个着点Y坐标;
    }
...
chessLogic.class.Man = function (key, x, y){
    ...
    this.show = function (){
        if (this.isShow) {
            if(!this.node){
                this.node = cc.instantiate(cc.find('Canvas/tmp/center/ChessBoard/'+chessLogic.args[this.pater].img))
                this.node.parent = cc.find('Canvas/tmp/center/ChessBoard')
            }
...
chessLogic.class.Pane = function (img, x, y){
    ...
    this.show = function (){
        if (this.isShow) {
            if(!this.node1){
                this.node1 = cc.instantiate(cc.find('Canvas/tmp/center/ChessBoard/r_box'));
                this.node1.setContentSize(22,22);
                cc.find('Canvas/tmp/center/ChessBoard').addChild(this.node1,-1)
            }
            ...
            if(!this.node2){
                this.node2 = cc.instantiate(cc.find('Canvas/tmp/center/ChessBoard/select_icon'))
                this.node2.setContentSize(22,22);
                cc.find('Canvas/tmp/center/ChessBoard').addChild(this.node2,-1)
            }
    ...
    this.show = function (){
        ...
        if(this.isShow){
            for (var i=0; i<this.dots.length;i++){
                if(!this.node_list[i]){
                    this.node_list[i] = cc.instantiate(cc.find('Canvas/tmp/center/ChessBoard/dot'))
                    this.node_list[i].setContentSize(22,22);
                    this.node_list[i].parent = cc.find('Canvas/tmp/center/ChessBoard')
                }
        ...
        if (this.isShowChoice) {
            if(!this.node1){
                this.node1 = cc.instantiate(cc.find('Canvas/tmp/center/ChessBoard/choice_icon'))
                this.node1.parent = cc.find('Canvas/tmp/center/ChessBoard')
            }
            this.node1.active = true;
            this.node1.position = cc.v2(chessLogic.spaceX * this.choiceX + chessLogic.pointStartX ,  520-chessLogic.spaceY * this.choiceY +chessLogic.pointStartY)
        }else{
            if(this.node1){
                this.node1.active = false;
            }
...
chessLogic.reset = function(rotation){
    ...
    for (const i in chessLogic.mans) {
        ...
        chessLogic.mans[i].rotation(rotation)
    }
    chessLogic.pane.isShow = false;
    this.show()
}
