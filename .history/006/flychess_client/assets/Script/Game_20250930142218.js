...
cc.Class({
    ...
    onGameStart(data){
        ...
        if(this.dice.active){
            this.dice.node.getChildByName("img_bg").getComponent(cc.Button).interactable = true;
        }
    ...
    showDiceNum(num,posId,cbFunc) {
        ...
        if(globalData.gameMgr.isRecover){
            this.dice.getComponent("Dice").showNum(num,posId,cbFunc)
        }else{
            this.dice.getComponent("Dice").playNum(num,posId,cbFunc)
        }
    },
