...
cc.Class({
    ...
    standUpChess(idx,cbFunc){
        ...
        let standOffsetY = 0;
        let chess_stand_list = [];
        for (const _idx in this.chess_steps) {
            if (this.chess_steps[_idx] == 0){
                standOffsetY += 10;
            }
        }
        //起
        ...
        for (const _idx in this.chess_steps) {
            if (this.chess_steps[_idx] == 0){
                chess_stand_list.push(this.chess_list[_idx]);
            }
        }
        ...
        chess_stand_list.sort(function(a, b){return a.position.y - b.position.y});
        for(var i=0;i<chess_stand_list.length;i++){
            chess_stand_list[i].setSiblingIndex(i);
        }
        // this.chess_list[this.cur_run_idx].setSiblingIndex(zidx+4);
        // console.log("zidx:",zidx);
    ...
    backHome(chess_idx){
        ...
        if(globalData.gameMgr.isRecover){
            ...
            this.chess_list[chess_idx].setRotation(0);
