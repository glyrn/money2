...
cc.Class({
    ...
    name:"PlayerNode",
    start(){
        this.dice.getComponent(cc.Animation).on('finished',  this.onFinished, this);
    },
    onFinished(){
    ...
    cleanDice(){
        this.dice.parent.node.active = false;
    },
    setTurnFlag(isShow){
    ...
    render(){
        ...
        if(data) {
            ...
            if (data.dice && data.dice > 0) {
                this.dice.parent.node.active = true;
                this.dice.getComponent(cc.Animation).play()
