...
var cards16 = [10,10,10,9,9,8,8,8,9];
...
function test(){
    let ret = validate_laizi(cards9,laizis);
    let type = ret.type;
    let len = ret.len;
    let key = ret.key;
    let lastCardInfo = {type:'AAAA',key:9};

    if (lastCardInfo.type === 'AAAA') {
        if (type === 'AAAA'){
        //硬炸
        if( lastCardInfo.isAAAA && has_laizi_num > 0 && ret.len > lastCardInfo.len ){
            return {
            status: true,
            key,
            type,
        ...
        if (type === 'AAAA') {
            ...
            if (type === lastCardInfo.type && ret.len === lastCardInfo.len && key > this.lastCardInfo.key) {
                return {
                    status: true,
                    key,
                    type,
