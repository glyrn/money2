    ...
    if(globalData.gameMgr.roomState.ctxCard.key > 0){
        ...
        if((globalData.gameMgr.roomState.ctxCard.type == "ABCDE" || 
            ...
            if(is_find == false){
                ...
                //如果对手是纯癞子 那么尝试找+1的软炸
                let offset = isAllLaizi ? 1:0;
                for (const value in check_card_map) {
                    console.log("检查AAA：",value , globalData.gameMgr.roomState.ctxCard.key)

                    if(check_card_map[value] == 4 && curLaiziCards.length + 4 >= check_match_len + offset && value > Math.ceil(globalData.gameMgr.roomState.ctxCard.key)){
                        hasAAA = true;
                        console.log("xx")
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 4 >= check_match_len + offset){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 4 + offset;i++){
                    ...
                    }else if(check_card_map[value] == 3 && value > Math.ceil(globalData.gameMgr.roomState.ctxCard.key)){
                        hasAAA = true;
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 3  >= check_match_len + offset){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 3 + offset;i++){
                                select_card_list.push(curLaiziCards[i]);
                            }
                            is_find = true;
                            break;
                    ...
                    }else if(check_card_map[value] == 2 && value > Math.ceil(globalData.gameMgr.roomState.ctxCard.key)){
                        hasAAA = true;
                        
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 2  >= check_match_len + offset){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 2 + offset;i++){
                                select_card_list.push(curLaiziCards[i]);
                            }
                            is_find = true;
                            break;
                    ...
                    }else if(check_card_map[value] == 1 && value > Math.ceil(globalData.gameMgr.roomState.ctxCard.key)){
                        hasAAA = true;
                        
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 1  >= check_match_len + offset){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 1 + offset;i++){
                                select_card_list.push(curLaiziCards[i]);
                            }
                            is_find = true;
                            break;
...
let playerData = {
    cards:[{value:3},{value:3},{value:3},{value:7},{value:7},{value:3},{value:12},{value:4},{value:12},{value:12}]
}
