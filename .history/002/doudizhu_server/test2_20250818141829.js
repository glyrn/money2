    ...
    if(globalData.gameMgr.roomState.ctxCard.key > 0){
        ...
        if((globalData.gameMgr.roomState.ctxCard.type == "ABCDE" || 
            ...
            ctxCards.forEach(_card=>{
                for(var laizi_card_key in laiziCards){
                    if(laiziCards[laizi_card_key].value == _card.value){
                        ...
                        console.log(laiziCards[laizi_card_key].value , _card.value)
                        isAllLaizi = false;
                    }
                }
            });
            ...
            if(is_find == false){
                ...
                console.log(isAllLaizi,offset)
                for (const value in check_card_map) {
                    console.log("检查AAA：",value , globalData.gameMgr.roomState.ctxCard.key)

                    if(check_card_map[value] == 3 && value > Math.ceil(globalData.gameMgr.roomState.ctxCard.key)){
                        hasAAA = true;
                        
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 3 + offset >= check_match_len){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 3;i++){
                        ...
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 2 + offset >= check_match_len){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 2;i++){
                                select_card_list.push(curLaiziCards[i]);
                            }
                            is_find = true;
                            break;
                        ...
                        //癞子+AAA的数量 等于 出牌数量
                        if(hasAAA && hasLaizi && curLaiziCards.length + 1 + offset >= check_match_len){
                            playerData.cards.forEach(_card=>{
                                if(_card.value == value){
                                    select_card_list.push(_card);
                                }
                            });
                            for(var i=0;i<check_match_len - 1;i++){
                                select_card_list.push(curLaiziCards[i]);
                            }
                            is_find = true;
                            break;
...
let playerData = {
    cards:[{value:14},{value:14},{value:14},{value:7},{value:7},{value:4},{value:4},{value:12},{value:12}]
}
