    ...
    renderCard(flag,playerData,roomState) {
        ...
        for (let i = 0; i < playerData.ctxCards.length; i++) {
            ...
            if(flag == "self")
            {
                ...
                this._cardOutList[i].position = cc.v2(21 + i * gap+base_out_pos.x,0+base_out_pos.y)
            }else if(flag == "right"){
                this._cardOutList[i].position = cc.v2(21 + i * gap+base_out_pos.x - total_out_width,0+base_out_pos.y)
            }
        }
    },
    //检测是否有特效牌型
