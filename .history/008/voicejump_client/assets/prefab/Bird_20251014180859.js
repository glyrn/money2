...
cc.Class({
    ...
    updatePosition(dt) {
        ...
        if (flying) {
            ...
            //限制不能超出屏幕
            this.node.parent.y = Math.min(this.node.parent.y, 540 / 2);

            if (this.node.parent.y < -540 / 2) {
                this.fallOver = true;
            }
        }
    },
        ...
        if (this.fallOver) {
            ...
            if(this.state !== State.Drop){
                ...
                if(!this.img_dead.active){
                    ...
                    // ]);
                    // var that = this;
                    var basePos = this.anim.node.position;
                    var seq2 = cc.sequence([
                        cc.moveTo(0.5, cc.v2(basePos.x, basePos.y+200)).easing(cc.easeOut(2.0)),
                        // cc.moveTo(0.4, cc.v2(basePos.x, basePos.y+100)),
                        cc.moveTo(1.3, cc.v2(basePos.x, basePos.y-380)).easing(cc.easeIn(8)),
