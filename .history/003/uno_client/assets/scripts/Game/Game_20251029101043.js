...
cc.Class({
    ...
    onLoad() {
        ...
        globalData.eventlister.on("GAME_START",function(data){
            that.reset()
            that.pushCardToDesk(data.top,-1,0)
        ...
        globalData.eventlister.on("PLAY_CARD_SUCCESS",function(data){
            ...
            that.pushCardToDesk(data.card,data.posId,data.plus_num);
            that.renderPlayer();
    ...
    pushCardToDesk(card,posId,plusNum){
        //记录当前出牌颜色、类型、位置
        ...
        //播放全局动画
        if(plusNum > 0){
            this.showGlobalEffect('anim+'+plusNum+"_"+globalData.gameMgr.getPlayerDataKey(globalData.gameMgr.playerData.turn),plusNum);
        }else if(card.value == 'turn'){
            this.showGlobalEffect("anim_turn")
        }else if(card.value == 'stop'){
            this.showGlobalEffect("anim_stop_"+globalData.gameMgr.getPlayerDataKey(globalData.gameMgr.playerData.turn));
        }
        if(!globalData.gameMgr.isRecover){
    ...
    showGlobalEffect:function(name,showGlobalEffect){
        //有动画
        if(!globalData.gameMgr.isRecover){
            let arr = name.split('anim+');
            if(arr[1] && parseInt(arr[1]) > 8){
                this.globalAnim.node.getChildByName("label").getComponent(cc.Label).string = "+"+arr[1];
                this.globalAnim.play('anim+n');
            }else{
